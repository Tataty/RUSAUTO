...
Adafruit_PWMServoDriver mss = Adafruit_PWMServoDriver(0x40);

#define SERVO_HEAD 0
#define SERVO_NECK 1
#define SERVO_ARM_LEFT 2
#define SERVO_ARM_RIGHT 3

#define PIN_TRIG1_MUSIC 2
#define PIN_TRIG2_MUSIC 4
...
void music_setup()
{
    pinMode(PIN_TRIG1_MUSIC,OUTPUT);
    pinMode(PIN_TRIG2_MUSIC,OUTPUT);
}
...
void servoWrite(int servo, int angle){
  mss.writeMicroseconds(servo, (angle * 1000) / 180 + 1000);
}
...
void servo_setup(){
    ...
    mss.begin();
    mss.setPWMFreq(50);
    servoWrite(SERVO_HEAD, 90);
    servoWrite(SERVO_NECK, 90);
    servoWrite(SERVO_ARM_LEFT, 90);
    servoWrite(SERVO_ARM_RIGHT, 90);
}
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    if (!strcmp(variable, "servohead")) {
        servoWrite(SERVO_HEAD, val);
    }
    else if (!strcmp(variable, "servoneck")) {
        servoWrite(SERVO_NECK, val);
    }
    else if (!strcmp(variable, "servoarmright")) {
        servoWrite(SERVO_ARM_RIGHT, val);
    }
    else if (!strcmp(variable, "servoarmleft")) {
        servoWrite(SERVO_ARM_LEFT, val);
    }
    else if (!strcmp(variable, "music")) {
        if (val == 0) {
          digitalWrite(PIN_TRIG1_MUSIC, HIGH);
          digitalWrite(PIN_TRIG2_MUSIC, HIGH);
        }
        else if (val == 1) {
          digitalWrite(PIN_TRIG1_MUSIC, LOW);
        }
        else if (val == 2) {
          digitalWrite(PIN_TRIG2_MUSIC, LOW);
        }
    }
